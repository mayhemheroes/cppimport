 ...
 * shebang wrapper, and fuzz-smoke.sh checks the ELF magic). cppimport is fuzzed through
 * its pure-Python templating layer, so the Atheris libFuzzer harness is a .py. This shim
 * is the ELF Mayhem launches; it immediately execs python3 <PY_SCRIPT> <args...>, handing
 * the libFuzzer/Atheris flags straight through. The Python process then IS the libFuzzer
 * target (it iterates inputs).
 *
...
#ifndef PY_SCRIPT
#define PY_SCRIPT "/mayhem/mayhem/fuzz_import.py"
#endif
